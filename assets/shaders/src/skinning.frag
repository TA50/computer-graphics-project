...
//
//
//    if(fragTan.xyz == vec3(0.0) || fragTan.w == 0.0) {
...
//        vec3 metalRoughColor = (Kd * lambert + CT) * lightColor * max(dot(L, N), 0.0);
////
////
//        vec3 Diffuse = texture(samplerColorMap, inUV).rgb * 0.975f * max(dot(N, L),0.0);
//        vec3 Specular = vec3(pow(max(dot(N, normalize(L + EyeDir)),0.0), 200.0));
//        Ambient = texture(samplerColorMap, inUV).rgb * 0.025f;
//
//        vec3 col  = (Diffuse + Specular) * lightColor + Ambient;
//
//
//        outFragColor = vec4(metalRoughColor + col, 1.0);
//    }
//}
