...
struct EarthTransformUniformBufferObject
{
	alignas(16) glm::mat4 mvpMat;
...
struct EarchShaderParamsObject
{
	alignas(4) float Power;
	alignas(4) float Ang;
...
protected:
	...
	DescriptorSetLayout DSLearth;	 // For earch
	...
	Model Mearth;
	Texture Tearth, TearthSpec, TearthNorm, TearthEmis, TearthClouds;
	...
	void localInit()
	{
		...
		DSLBlinn.init(this, {{0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_VERTEX_BIT, sizeof(BlinnUniformBufferObject), 1},
							 {1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 0, 1},
							 {2, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(BlinnMatParUniformBufferObject), 1}});

		DSLEmission.init(this, {{0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_VERTEX_BIT, sizeof(EmissionUniformBufferObject), 1},
								{1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 0, 1}});
		...
		// int linkSize;
		// int count;
		DSLearth.init(this, {{0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_VERTEX_BIT, sizeof(EarthTransformUniformBufferObject), 1},
							 {1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 0, 1},
							 {2, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 1, 1},
							 ...
							 {6, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(EarchShaderParamsObject), 1}});

		// Vertex descriptors
		VDBlinn.init(this, {{0, sizeof(BlinnVertex), VK_VERTEX_INPUT_RATE_VERTEX}},
		...
		VDEarth.init(this, {{0, sizeof(EarthVertex), VK_VERTEX_INPUT_RATE_VERTEX}},
		...
		// Pipelines [Shader couples]
		PBlinn.init(this, &VDBlinn, "assets/shaders/bin/BlinnShader.vert.spv", "assets/shaders/bin/BlinnShader.frag.spv", {&DSLGlobal, &DSLBlinn});
		PEmission.init(this, &VDEmission, "assets/shaders/bin/EmissionShader.vert.spv", "assets/shaders/bin/EmissionShader.frag.spv", {&DSLEmission});
		PskyBox.init(this, &VDskyBox, "assets/shaders/bin/SkyBoxShader.vert.spv", "assets/shaders/bin/SkyBoxShader.frag.spv", {&DSLskyBox});

		PskyBox.setAdvancedFeatures(VK_COMPARE_OP_LESS_OR_EQUAL, VK_POLYGON_MODE_FILL,
									VK_CULL_MODE_BACK_BIT, false);
		// **A10** Place here the initialization of the pipeline. Remember that it should use shaders in files
		//		"assets/shaders/bin/NormalMapVert.spv" and "assets/shaders/bin/NormalMapFrag.spv", it should receive the new VertexDescriptor you defined
		//		And should receive two DescriptorSetLayout, the first should be DSLGlobal, while the other must be the one you defined

		Pearth.init(this, &VDEarth, "assets/shaders/bin/NormalMapShader.vert.spv", "assets/shaders/bin/NormalMapShader.frag.spv", {&DSLGlobal, &DSLearth});
		// Create models
		Mship.init(this, &VDBlinn, "assets/models/X-WING-baker.obj", OBJ);
		Msun.init(this, &VDEmission, "assets/models/Sphere.obj", OBJ);
		MskyBox.init(this, &VDskyBox, "assets/models/SkyBoxCube.obj", OBJ);

		// **A10** Place here the loading of the model. It should be contained in file "assets/models/Sphere.gltf", it should use the
		//		Vertex descriptor you defined, and be of GLTF format.
		Mearth.init(this, &VDEarth, "assets/models/Sphere.gltf", GLTF);

		// Create the textures
		Tship.init(this, "assets/textures/XwingColors.png");
		Tsun.init(this, "assets/textures/2k_sun.jpg");
		...
		Tearth.init(this, "assets/textures/2k_earth_daymap.jpg");

		// Specular color of the planet in: "2k_earth_specular_map.png"
		TearthSpec.init(this, "assets/textures/2k_earth_specular_map.png");
		...
		// the init function should be the following: .init(this, "assets/textures/2k_earth_normal_map.png", VK_FORMAT_R8G8B8A8_UNORM);
		TearthNorm.init(this, "assets/textures/2k_earth_normal_map.png", VK_FORMAT_R8G8B8A8_UNORM);

		// Emission map in: "2k_earth_nightmap.jpg"
		TearthEmis.init(this, "assets/textures/2k_earth_nightmap.jpg");

		// Clouds map in: "2k_earth_clouds.jpg"
		TearthClouds.init(this, "assets/textures/2k_earth_clouds.jpg");

		// Descriptor pool sizes
		// WARNING!!!!!!!!
		// Must be set before initializing the text and the scene
		// **A10** Update the number of elements to correctly size the descriptor sets pool
		DPSZs.uniformBlocksInPool = 5 + 2; // 2 more uniform blocks
		DPSZs.texturesInPool = 4 + 5;	   // 5 textures
		DPSZs.setsInPool = 4 + 1;		   // 1 more descriptor sets
	...
	void pipelinesAndDescriptorSetsInit()
	{
		...
		DSearth.init(this, &DSLearth, {&Tearth, &TearthSpec, &TearthNorm, &TearthEmis, &TearthClouds});
	...
	void populateCommandBuffer(VkCommandBuffer commandBuffer, int currentImage)
	{
		...
		std::cout << "Binding Earth Earth: " << "\n";
		DSearth.bind(commandBuffer, Pearth, 1, currentImage);
		std::cout << "Drawing Earth: " << "\n";
		vkCmdDrawIndexed(commandBuffer,
	...
	void updateUniformBuffer(uint32_t currentImage)
	{
		...
		EarthTransformUniformBufferObject earthUbo{};
		...
		EarchShaderParamsObject earthMatParUbo{};

		earthMatParUbo.Power = 200.0;
		...
		earthMatParUbo.Ang = tTime * TangTurnTimeFact;
		...
		earthMatParUbo.ShowCloud = ShowCloud;
		// The selector for showing the clouds of the uniform buffer containing the material parameters of the new object should be set to:
